...
class NN:
    ...
    def __init__(self, P_back, x, P, rho, u, E, layers):
        ...
        self.loss = tf.reduce_sum(tf.square(self.P_tf - self.P_pred))/(P_norm**2) + \
                    ...
                    50*tf.reduce_sum(tf.square(self.e2))/(e2_norm**2) + \
                    50*tf.reduce_sum(tf.square(self.e3))/(e3_norm**2) + \
                    50*tf.reduce_sum(tf.square(self.e1))/(e1_norm**2) + \
                    50*tf.reduce_sum(tf.square(self.e4))/(P_norm**2)
...
if __name__ == "__main__":
    ...
    S = 1 + 2.2*(3*z-1.5)**2
    mass_train = rho*u*S
    for i in range (0, 1212, 303):
        plt.plot(z[i:i+101], mass_train[i:i+101], 'b', label='mass_rate')
        plt.title('Mass')
    ...
    for i in range (0, 606, 101):
        plt.plot(z_test[i:i+101], P_pred[i:i+101], 'm', label='NN')
        plt.plot(z_test[i:i+101], P_test[i:i+101], 'g', label='Truth')
        plt.title('Comparison of NN and Ground Truth results for Pressure')
        plt.xlabel('Nozzle Length')
        plt.ylabel('value')
        plt.legend()
    plt.show()
    for i in range (0, 606, 101):
        plt.plot(z_test[i:i+101], E_pred[i:i+101], 'bx', label='NN')
        plt.plot(z_test[i:i+101], E_test[i:i+101], 'g', label='Truth')
        plt.title('Comparison of NN and Ground Truth results for Energy')
        plt.xlabel('Nozzle Length')
        plt.ylabel('value')
        plt.legend()
    plt.show()
    for i in range (0, 606, 101):
        plt.plot(z_test[i:i+101], u_pred[i:i+101], 'cx', label='NN')
        plt.plot(z_test[i:i+101], u_test[i:i+101], 'g', label='Truth')
        plt.title('Comparison of NN and Ground Truth results for speed')
        plt.xlabel('Nozzle Length')
        plt.ylabel('value')
        plt.legend()
    plt.show()
    for i in range (0, 606, 101):
        plt.plot(z_test[i:i+101], rho_pred[i:i+101], 'yx', label='NN')
        plt.plot(z_test[i:i+101], rho_test[i:i+101], 'g', label='Truth')
        plt.title('Comparison of NN and Ground Truth results for density')
        plt.xlabel('Nozzle Length')
        plt.ylabel('value')
        plt.legend()
    plt.show()
    ...
    # mass
    mass = rho_test*u_test*S
    for i in range (0, 606, 101):
        plt.plot(z_test[i:i+101], mass[i:i+101], 'm', label='mass_rate')
        plt.title('Mass')
        plt.xlabel('Nozzle Length')
        plt.ylabel('value')
        plt.legend()
    ...
    # energy
    energy = (rho_test*E_test + P_test)*u_test*S
    for i in range (0, 606, 101):
        plt.plot(z_test[i:i+101], energy[i:i+101], 'b', label='energy_rate')
        plt.title('Energy')
        plt.xlabel('Nozzle Length')
        plt.ylabel('value')
        plt.legend()
